...
export async function detailsPage(ctx) {
    ...
    const [book, likes, hasLike] = await Promise.all([
        ...
        userData ? getMyLikeByBookId(ctx.params.id, userData.id) : 0 //Ако влезе guest ще върне нулл и ще гръмне ако няма тернарен оператор
    ]);
    ...
    async function onDelete() {
        const choice = confirm(`Are you sure you want to delete this ${book.title}`);
        if (choice) {
