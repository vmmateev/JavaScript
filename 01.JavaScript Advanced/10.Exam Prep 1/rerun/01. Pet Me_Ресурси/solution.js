function solve() {
    ...
    function addPet(event) {
        ...
        // pet.innerHTML = `<strong>${name}</strong>`;
        ...
        input.kind.value = '';
        input.owner.value = '';
        //Array.from(fields).foreach(field => field.value=''); // изчистване на всички полета
        // Може и form.reset но трявба да имаме референция към него  document.querySelector('form').formReset()
    ...
    function e(type, attr, ...content) {
        ...
        for (let item of content) {
            if (typeof item == 'string' || typeof item == 'number') {// item не приема стрингове или числа за това го превръщаме в node елемент
